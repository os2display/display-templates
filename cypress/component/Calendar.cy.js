...
// Because the Calendar template results are affected by when the events take place,
// it is necessary to control the time in the tests.
// Use fixed date for data and asserts to ensure consistent results.
const timestampForData = 60 * 60 * 8 * 1000 + 60 * 60 * 24 * 7 * 1000;
const dateForData = new Date(timestampForData);

describe("Calendar", () => {
  it("Calls slideDone correctly", () => {
    const mock = {
      slideDone: (arg) => {
        return arg;
      },
  ...
  it("Calendar multiple days", () => {
    ...
    cy.mount(
      <div className="slide" id="SLIDE_ID">
        <Calendar
            ...
            feedData: [
              {
                ...
                startTime: dayjs(dateForData).subtract(30, "minutes").unix(),
                endTime: dayjs(dateForData).subtract(15, "minutes").unix(),
              ...
              },
              {
                ...
                title: "Cake is gone",
                startTime: dayjs(dateForData).subtract(30, "minutes").unix(),
                endTime: dayjs(dateForData).add(2, "hour").unix(),
                resourceTitle: "Det fulde rum",
                resourceId: "MUendelig",
              },
              {
              ...
              },
              {
                ...
                startTime: dayjs(dateForData).add(1, "hour").add(30, "minutes").unix(),
                endTime: dayjs(dateForData).add(2, "hour").unix(),
                resourceTitle:
              ...
              },
              {
                id: "uniqueEvent2",
                //        title: "Cookies",
                startTime: dayjs(dateForData).add(2, "hour").unix(),
                endTime: dayjs(dateForData).add(3, "hour").unix(),
              ...
              },
              {
                id: "uniqueEvent3",
                title: "Coffee",
                startTime: dayjs(dateForData).add(3, "hour").add(15, "minutes").unix(),
                endTime: dayjs(dateForData).add(4, "hour").unix(),
              ...
              },
              {
                id: "uniqueEvent4",
                title: "Tea",
                startTime: dayjs(dateForData).add(5, "hour").unix(),
                endTime: dayjs(dateForData).add(8, "hour").unix(),
                resourceTitle: null,
                resourceId: "M4",
              },
              {
              ...
              },
              {
                id: "uniqueEvent20",
                title: "Tea",
                startTime: dayjs(dateForData).add(1, "hour").add(1, "day").unix(),
                endTime: dayjs(dateForData).add(2, "hour").add(1, "day").unix(),
                resourceTitle: null,
                resourceId: "M4",
              },
              {
                id: "uniqueEvent21",
                title: "Tea",
                startTime: dayjs(dateForData).add(3, "hour").add(3, "day").unix(),
                endTime: dayjs(dateForData).add(4, "hour").add(3, "day").unix(),
                resourceTitle: null,
                resourceId: "M4",
              },
              {
                id: "uniqueEvent22",
                title: "Tea",
                startTime: dayjs(dateForData).add(2, "hour").add(6, "day").unix(),
                endTime: dayjs(dateForData).add(3, "hour").add(6, "day").unix(),
                resourceTitle: null,
                resourceId: "M4",
              },
              {
                id: "uniqueEvent23",
                title: "Tea",
                startTime: dayjs(dateForData).add(2, "hour").add(8, "day").unix(),
                endTime: dayjs(dateForData).add(3, "hour").add(8, "day").unix(),
                resourceTitle: "Test",
                resourceId: "M5",
              },
              {
          ...
          run={dayjs(dateForData).toISOString()}
          slideDone={() => {}}
          executionId="SLIDE_ID"
        />
      </div>
    ...
    cy.get("section")
      ...
      .should(
        "have.text",
        dayjs(dateForData).locale(localeDa).format("dddd D. MMMM")
      );
    cy.get("section")
      .eq(2)
      .find("p")
      .should(
        "have.text",
        dayjs(dateForData).add(1, "day").locale(localeDa).format("dddd D. MMMM")
      );
    cy.get("section")
      .eq(3)
      .find("p")
      .should(
        "have.text",
        dayjs(dateForData).add(3, "days").locale(localeDa).format("dddd D. MMMM")
      );
    cy.get("section")
      .eq(4)
      .find("p")
      .should(
        "have.text",
        dayjs(dateForData).add(6, "day").locale(localeDa).format("dddd D. MMMM")
      );
    ...
    cy.get("section")
      ...
      .should(
        "have.text",
        `${dayjs(dateForData).locale(localeDa).add(2, "hour").format("HH:mm")}`
      );
    cy.get("section")
      .eq(0)
      .find("article")
  ...
  it("Calendar multiple", () => {
    ...
    cy.get(".header-date").contains(
      dayjs(dateForData).locale(localeDa).format("dddd D. MMMM HH:mm"),
      { matchCase: false }
    );
    cy.get(".content-item")
      .eq(0)
    ...
    cy.get(".content-item-time")
      ...
      .should(
        ...
        `${dayjs(dateForData)
          .subtract(30, "minutes")
          .locale(localeDa)
          .format("HH:mm")} - ${dayjs(dateForData)
          .add(2, "hour")
          .locale(localeDa)
          .format("HH:mm")}`
      );
    ...
    cy.get(".single--next")
      ...
      .should(
        ...
        `${dayjs(dateForData)
          .add(1, "hour")
          .add(30, "minutes")
          .format("HH:mm")} - ${dayjs(dateForData).add(2, "hour").format("HH:mm")}Cake`
      )
      .should("have.css", "font-size")
      .should("include", "36px");
    cy.get(".single--next")
      ...
      .should(
        "have.text",
        `${dayjs(dateForData).add(2, "hour").format("HH:mm")} - ${dayjs(dateForData)
          .add(3, "hour")
